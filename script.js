...
fetch(PAGE_URL) // hacemos la solicitud para la url lo que nos da una respuesta
  .then((response) => {
    // este primer then maneja esta respuesta de la solicitudgit
    // aca verificamos si la respuesta es exitosa 200
    if (!response.ok) {
      throw new Error("Error en de red" + response.status);
    }
  ...
  .then((data) => {
    // este then maneja el contenido de la pagina
    console.log(data);
  })

  .catch((error) => {
    // captura y maneja cualquier error que pueda ocurrir durante el proceso de solicitud
